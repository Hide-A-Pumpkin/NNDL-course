...
for i in range(args.runs):
    ...
    if few_shot:
        for index in range(len(args.n_shots)):
            best_val_acc,_=stats(np.array(run_stats["best_val_acc"])[:,index], "")
            shotInfo="{:d}-shot".format(args.n_shots[index])
            print(shotInfo,100*best_val_acc)
            if args.wandb:
                wandb.log({"run": i+1,"test acc {:d}-shot".format(args.n_shots[index]):np.mean(np.array(run_stats["best_val_acc"])[:,index])})
