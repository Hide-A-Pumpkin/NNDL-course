...
def generate_float(img,row_block_num,offset):
    '''
    用于GUI生成图片函数
    input: target img
    output: float img
    ...
    grid_points = init_param(row_block_num,col_block_num,-offset,offset)
...
#创建数据增强的文件夹在new_path，原始训练集在path/train
path='./img/oracle_200_%s_shot'%(str(shot))
name='FFD_train_%s_block%s_%s_%s_shot'%(str(offset),str(block_num),str(num),str(shot))
new_path=os.path.join(path,name)
os.makedirs(new_path )

#遍历原始训练集下的所有文件夹，并生成数据增强图片
for file in os.listdir(os.path.join(path,'train')):
    cur_path = os.path.join(path, 'train',file)
    print(file)
    ...
    for i,img_file in enumerate(os.listdir(cur_path)):
        ...
        for j in range(num*i,num*i+num):
            float=generate_float(source,row_block_num=block_num,offset=offset)
